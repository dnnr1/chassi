import { validateVin, isValidVin, normalizeVin, isValidVinStructure } from '../src/core/validateVin';
...
describe('validateVin', () => {
  describe('valid VINs', () => {
    ...
    validVins.forEach(vin => {
      it(`should validate ${vin}`, () => {
        ...
        expect(result.normalizedVin).toBe(vin);
      });
    });
  });
  ...
  describe('invalid VINs', () => {
    ...
    it('should reject VIN with invalid check digit', () => {
      const result = validateVin('9BWZZZ370VT004251');
      expect(result.valid).toBe(false);
